...
for max_argsort_count in range(1, comb(n, 2) + 5):
    ...
    np.testing.assert_allclose(U @ np.diag(S) @ V, covecs)
    k = 1
    while not (np.sign(U[:, :k] @ V[:k, :]) == covecs).all() and k < min(covecs.shape):
        k += 1
    print(k, min(covecs.shape), sep="\t")
